...
function RecordDialog({ open, initial, onClose, onSave }: { open: boolean; initial: Observation; onClose: () => void; onSave: (observation: Observation) => void }) {
  ...
  return <dialog ref={dialog} className="record-dialog" onCancel={event => { event.preventDefault(); onClose() }} aria-labelledby="record-title">
    <div className="dialog-shell">
      <header><div><span className="eyebrow">Registro de hoje · etapa {step + 1}/4</span><h2 id="record-title">{titles[step]}</h2></div><button className="icon-button" onClick={onClose} aria-label="Fechar registro">×</button></header>
      <div className="progress" aria-label={`Passo ${step + 1} de 4`}><span style={{ width: `${(step + 1) * 25}%` }}/></div>
      <div className="dialog-content">
        {step === 0 && <fieldset><legend className="sr-only">Sensação na vulva ao longo do dia</legend><p className="prompt-help">Considere o que você percebeu durante suas atividades normais.</p><div className="choice-list">{sensationOptions.map(option => <label key={option.value} className={draft.sensation === option.value ? 'selected' : ''}><input type="radio" name="sensation" value={option.value} checked={draft.sensation === option.value} onChange={() => setDraft({ ...draft, sensation: option.value })}/><span className="choice-mark"><Icon name="check" size={16}/></span><span><strong>{option.label}</strong><small>{option.description}</small></span></label>)}</div>{draft.sensation === 'other' && <label className="text-field"><span>Descreva a sensação</span><input value={draft.sensationRaw ?? ''} onChange={event => setDraft({ ...draft, sensationRaw: event.target.value })} placeholder="Use suas próprias palavras"/></label>}</fieldset>}
      ...
      <footer><button className="dialog-back" onClick={() => step === 0 ? onClose() : setStep(step - 1)}>{step === 0 ? 'Cancelar' : 'Voltar'}</button>{step < 3 ? <button className="dialog-next" onClick={() => setStep(step + 1)}>Continuar <Icon name="arrow" size={18}/></button> : <button className="dialog-next" onClick={save}>Salvar registro <Icon name="check" size={18}/></button>}</footer>
    </div>
  </dialog>
}
